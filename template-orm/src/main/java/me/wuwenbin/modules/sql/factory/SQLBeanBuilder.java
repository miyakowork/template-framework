...
public final class SQLBeanBuilder {
     ...
     * @return {@link String}
     */
    public String selectPart(int... routers) {
        if (!SQLBuilderUtils.SQLTableIsExist(beanClass)) {
            throw new SQLTableNotFoundException(beanClass);
        } else {
            StringBuilder sb = new StringBuilder(selectPre);
                ...
                for (Field field : fields) {
                    ...
                    if (isSQLColumnAndPk || isSQLPk) {
                        ...
                        if (SQLBuilderUtils.canBeInsert(field)) {
                            ...
                            boolean isFieldInRouters = SQLBuilderUtils.fieldRoutersInParamRouters(SQLBuilderUtils.getRouterInField(field), routers);
                            boolean isNeedInsert = !isSqlColumnAnnotationPresent || !field.getAnnotation(sqlColumnClass).pk();
                            if (isFieldInRouters && isNeedInsert) {
